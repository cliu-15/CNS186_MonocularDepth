...
#from models_resnet_original import Resnet50_md
...
class Model:
    ...
    def __init__(self, args):
        ...
        self.model = Resnet50(3) #get_model(args.model, input_channels=args.input_channels, pretrained=args.pretrained)
    ...
    def test(self):
        ...
        for i in range(20):
            print(i)
            plt.imshow(np.squeeze(disparities[i,:,:]), cmap='plasma')
            plt.axis('off')
            #plt.show()
            plt.savefig('data/output/images/kittiphone_'+str(i)+'_disp.png')
            plt.imshow(np.squeeze(disparities_pp[i,:,:]), cmap='plasma')
            plt.axis('off')
            #plt.show()
            plt.savefig('data/output/images/kitti_phone_'+str(i)+'_disp_pp.png')

        print('Finished Testing')

# dict_parameters = edict({'data_dir':'data/kitti/train/',
#                          'val_data_dir':'data/kitti/val/',
#                          'model_path':'data/models/monodepth_resnet50_001_test_newest_cpt.pth',
...
dict_parameters = edict({'data_dir':'data/kitti/train/',
                         'val_data_dir':'data/kitti/val/',
                         'model_path':'data/models/monodepth_resnet50_001_test_ours_cpt.pth',
                         'output_directory':'data/output/',
                         'input_height':256,
                         'input_width':512,
                         'model':'resnet50_md',
...
dict_parameters_test = edict({'data_dir':'data/kitti/test',
                              'model_path':'data/models/monodepth_resnet50_001_test_newest_cpt_KITTI_FINAL.pth',
                              'output_directory':'data/output/',
                              'input_height':256,
                              'input_width':512,
                              'model':'resnet50_md',
...
with torch.no_grad():
    for data in model_test.loader:
        data = to_device(data,model_test.device)
        left = data.squeeze()
        plt.imshow(np.transpose(left[0].cpu().detach().numpy(),(1,2,0)))
        plt.show()
        disps = model_test.model(left)
        disp_l = [d[:,0,:,:].unsqueeze(1) for d in disps]
        left = np.squeeze(np.transpose(disp_l[0][0,:,:,:].cpu().detach().numpy(), (1,2,0)))
        print(left.shape)
        plt.imshow(left, cmap='plasma')
        plt.show()
